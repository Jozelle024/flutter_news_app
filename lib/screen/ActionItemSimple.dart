...
import 'package:flutter_news_app/provider/LayoutSizeChangeNotification.dart';
...
class _OnSlideSimpleState extends State<OnSlideSimple> {
  ...
  Widget build(BuildContext context) {
    ...
    for (ActionItemsSimple item in widget.items) {
      ...
      scrollWidgets.add(InkWell(
          ...
          onTap: () {
              scheduleMicrotask((){
                controller.animateTo(10.0, duration: Duration(milliseconds: 600), curve: Curves.decelerate);
              });
            item.onPress();
          }));
    }
  ...
  bool _handleNotification(dynamic notification){
    if(notification is ScrollEndNotification){
      if(notification.metrics.pixels > 0.0 && notification.metrics.pixels < (ITEMS_WIDTH/2))
        controller.animateTo(0.0, duration: const Duration(milliseconds: 800), curve: Curves.bounceOut);
      else if (notification.metrics.pixels > 0.0 && notification.metrics.pixels < (ITEMS_WIDTH / 2))
        scheduleMicrotask(() {  
